...
// ========== PCG FILTER SETUP (Simple Butterworth Implementation) ==========
...
public:
  SimpleLowPassFilter(float cutoff_hz, float sample_rate_hz) {
...
SimpleLowPassFilter lowPassFilter(PCG_CUTOFF_HZ, PCG_SAMPLE_RATE);
...
LSM9DS1 imu;
MAX30205 tempSensor;
MAX30003 ecgSensor(ECG_CS_PIN);  // Pass CS pin to constructor
...
void pcgThread() {
  ...
  while (true) {
    ...
    if (currentTime - lastSample >= 1) {  // 1ms sampling
      ...
      float filtered = lowPassFilter.filter(amplified);  // Use .filter() method
...
void ecgThread() {
  ...
  while (true) {
    ...
    if (currentTime - lastSample >= 8) {  // 125Hz sampling (8ms)
      ...
      if (ecgReady) {
        int32_t ecgValue = 0;
        bool success = ecgSensor.readEcgSample(ecgValue);
...
bool validateSensors() {
  ...
  if (ecgReady) {
    delay(100);
    int32_t testValue = 0;
    bool ecgTest = ecgSensor.readEcgSample(testValue);
    if (ecgTest) {
      ecgLastUpdate = millis();
      Serial.println("OK");
    } else {
...
void setup() {
  ...
  bool ecgInitSuccess = ecgSensor.begin();
  delay(100);
